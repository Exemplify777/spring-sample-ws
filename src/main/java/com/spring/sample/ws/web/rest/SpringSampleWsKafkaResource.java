...
@RestController
@RequestMapping("/api/spring-sample-ws-kafka")
public class SpringSampleWsKafkaResource {

    private final Logger log = LoggerFactory.getLogger(SpringSampleWsKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public SpringSampleWsKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
