...
import com.spring.sample.ws.config.KafkaSseConsumer;
import com.spring.sample.ws.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SpringSampleWsKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/spring-sample-ws-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/spring-sample-ws-kafka/consume")
